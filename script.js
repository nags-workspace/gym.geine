document.addEventListener('DOMContentLoaded', () => {
    ...
    const loadHistoryFromCloud = async () => {
        ...
        try {
            ...
            alert("Could not connect to cloud storage. Using local backup.");
        }
    };
    ...
    const updateChoiceButtonsState = () => { const history = loadHistory(); const todayString = new Date().toDateString(); const loggedToday = history.filter(item => new Date(item.timestamp).toDateString() === todayString).map(item => item.workoutName); const choiceButtons = document.querySelectorAll('#workout-choices .workout-choice'); choiceButtons.forEach(button => { const workoutName = button.dataset.workout; if (loggedToday.includes(workoutName)) { button.disabled = true; button.classList.add('disabled'); } else { button.disabled = false; button.classList.remove('disabled'); } }); };
    const renderHistory = () => { const history = loadHistory(); historyList.innerHTML = ''; const groupedByDate = history.reduce((acc, item) => { const date = new Date(item.timestamp).toDateString(); if (!acc[date]) acc[date] = []; acc[date].push(item); return acc; }, {}); const sortedDates = Object.keys(groupedByDate).sort((a, b) => new Date(b) - new Date(a)); sortedDates.forEach(dateString => { const workouts = groupedByDate[dateString].sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp)); const dateCard = document.createElement('li'); dateCard.className = 'date-group-card'; const dateHeader = document.createElement('h3'); dateHeader.className = 'date-header'; dateHeader.textContent = new Date(dateString).toLocaleDateString(undefined, { weekday: 'long', month: 'long', day: 'numeric' }); dateCard.appendChild(dateHeader); workouts.forEach(item => { const workoutEntry = document.createElement('div'); workoutEntry.className = 'workout-entry'; workoutEntry.innerHTML = `<div class="workout-entry-details"><div class="name">${item.workoutName}</div><div class="time">${new Date(item.timestamp).toLocaleTimeString(undefined, { hour: 'numeric', minute: '2-digit' })}</div></div><div class="history-item-actions"><button class="icon-btn edit-log-btn" data-timestamp="${item.timestamp}" title="Edit Entry"><i class="fas fa-pencil-alt"></i></button></div>`; dateCard.appendChild(workoutEntry); }); historyList.appendChild(dateCard); }); };
